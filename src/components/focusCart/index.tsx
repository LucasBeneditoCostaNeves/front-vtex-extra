...
export const FocusCart = () => {
  ...
  const arraySemRepeticao: any = [];
  const nomesVistos: any = [];
  ...
  const prices = cart.map((element: any) => Number(element.price));
  console.log(prices);

  let novoReduce = prices.reduce((acumulador: any, valorAtual: any) => {
    ...
    <DivStyled>
      <div className="white">
        ...
        {arraySemRepeticao.length > 0 ? (
          arraySemRepeticao && (
            <>
              {arraySemRepeticao.map((element: any) => CardCart(element))}
          ...
          <>
            ...
            <h3 className="text-null-2">Que tal adicionar novos itens?</h3>
          </>
        )}
      </div>
    </DivStyled>
